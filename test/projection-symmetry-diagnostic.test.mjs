...
function preserves(nodes,segments,angle,tol,mode){const map=new Array(nodes.length),used=new Set();for(let i=0;i<nodes.length;i++){const p=transform(angle,nodes[i].xy);let bi=-1,bd=Infinity;for(let j=0;j<nodes.length;j++){if(used.has(j)||sig(nodes[i],mode)!==sig(nodes[j],mode))continue;const d=Math.hypot(nodes[j].xy[0]-p[0],nodes[j].xy[1]-p[1]);if(d<bd){bd=d;bi=j}}if(bi<0||bd>tol)return false;map[i]=bi;used.add(bi)}for(const k of segments){const[a,b]=k.split(':').map(Number);if(!segments.has(edgeKey(map[a],map[b])))return false}return true}
function axes(nodes,segments,scale,tolFactor,mode){const cand=[];const add=a=>{a=norm(a);if(!cand.some(x=>adist(x,a)<1e-5))cand.push(a)};for(let i=0;i<nodes.length;i++){const ai=Math.atan2(nodes[i].xy[1],nodes[i].xy[0]);add(ai);for(let j=i+1;j<nodes.length;j++){if(sig(nodes[i],mode)!==sig(nodes[j],mode))continue;const aj=Math.atan2(nodes[j].xy[1],nodes[j].xy[0]);const b=Math.atan2(Math.sin(ai+aj),Math.cos(ai+aj))/2;add(b);add(b+Math.PI/2)}}const pass=cand.filter(a=>preserves(nodes,segments,a,scale*tolFactor,mode)).sort((a,b)=>a-b);const out=[];for(const a of pass)if(!out.some(x=>adist(x,a)<Math.PI/180))out.push(a);return out.map(a=>Number((a*180/Math.PI).toFixed(2)))}

test('symmetry variants',()=>{for(const solid of projections.solids){const wanted=targets.get(solid.name);if(!wanted)continue;const vs=views.solids.find(v=>v.name===solid.name),g=geometryForSolid(solid.name);for(const item of solid.classes){if(!wanted.has(item.role.name))continue;const v=vs.views.find(x=>x.classId===item.id);const a=arrangement(g.vertices,g.edges,viewFrame(v.viewDirection,v.rollDegrees));const results={};for(const mode of ['strict','degree','none'])for(const t of [0.003,0.006,0.01,0.02,0.04])results[`${mode}-${t}`]=axes(a.nodes,a.segments,a.scale,t,mode);console.log(JSON.stringify({name:`${solid.name}-${item.role.name}`,results}))}}});
